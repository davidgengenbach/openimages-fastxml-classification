function score_mat = fastXML_test(tst_X_Xf, param, model_folder)
    ...
    cmd = sprintf('fastXML_test %s %s %s %s', file_tst_X_Xf, file_score_mat, model_folder, get_arguments(param));
    if isunix
        cmd = ['./' cmd];
    end
