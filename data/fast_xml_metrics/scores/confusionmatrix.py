...
class ConfusionMatrix(dict):
    ...
    def __str__(self):
        ...
        s = max([max([0]+[len(l) for l in labels if isinstance(l, str)]+[len(str(x)) for x in self.columntotals.values()]), 7])
    ...
    def _light_single_add(self, g, p):
        ...
        # Fill
        if g == p:
            cache_key = u'tp_'+str(p)
            self.cached[cache_key] = self.cached.get(cache_key, 0) + 1
        else:
            cache_key = u'fp_'+str(p)
            self.cached[cache_key] = self.cached.get(cache_key, 0) + 1
            cache_key = u'fn_'+str(g)
            self.cached[cache_key] = self.cached.get(cache_key, 0) + 1
