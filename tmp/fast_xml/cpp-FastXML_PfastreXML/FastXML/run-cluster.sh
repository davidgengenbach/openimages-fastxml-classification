...
score_file="${results_dir}/${dataset}_results.txt"

NUM_THREADS=1

# training
if [ "$MODE" == "train" ]; then
	./fastXML_train $trn_ft_file $trn_lbl_file $model_dir -T $NUM_THREADS -s 0 -t 10 -b 1.0 -c 1.0 -m 50 -l 10
fi
...
# testing
if [ "$MODE" == "test" ]; then
	./fastXML_test $trn_ft_file $score_file $model_dir -T $NUM_THREADS
fi
